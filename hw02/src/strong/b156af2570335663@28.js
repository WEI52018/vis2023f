...
function _6(yCounts,starSign,data)
{
  ...
  for (var i = 0; i <= 11; i++) { 
    ...
    yCounts.push({Constellation:starSign[i], gender:"male", count:0}); 
    //Object包含：1. 星座，2.男性，3.人數(設為0)
    yCounts.push({Constellation:starSign[i], gender:"female", count:0}); 
...
Plot.plot({
  ...
  marks: [
    Plot.ruleY([0]),
    Plot.barY(yCounts, {x: "Constellation", y: "count", tip: true , sort: {x: {starSign}}, fill:"gender"}),
  ]
})
)}
